...
class SleepyThread(Thread):
    ...
    def __init__(self, number, sleepMax):
        """Create a thread with the given name and a random sleep interval less than the maximum."""
        Thread.__init__(self, name = "Thread " + str(number))
        self.sleepInterval = random.randint(1, sleepMax)

    def run(self):
        """Print the thread's name and sleep interval and sleep for that interval. Print the name again at wake-up."""
        print("%s starting, with sleep interval: %d seconds" % (self.name, self.sleepInterval))
        time.sleep(self.sleepInterval)
        print("%s waking up" % self.name)

def main():
    """Create the user's number of threads with sleep intervals less than the user's maximum. Then start the threads."""
